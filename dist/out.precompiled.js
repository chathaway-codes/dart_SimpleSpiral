...
Interceptor: {"": "Object;",
  ...
  "%": "DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGPointList|SVGRect|WebGLBuffer|WebGLProgram|WebGLShader"
},
...
JSArray: {"": "List/Interceptor;",
  ...
  addAll$1: function(receiver, collection) {
    var t1;
    for (t1 = new H.ListIterator(collection, 3, 0, null); t1.moveNext$0();)
...
InputElement: {"": "HtmlElement;height=,width=", $isInputElement: true, "%": "HTMLInputElement"},
...
UniformLocation: {"": "Interceptor;", "%": "WebGLUniformLocation"}}],
...
["simplespiral", "../app/simplespiral.dart", , M, {
main: function() {
  var t1, t2;
  P.print(document.querySelector("#degrees").getAttribute("value"));
  ...
  t1.setIdentity$0();
  $.sMatrix = t1;
  $.gl = J.getContext3d$0$x($.get$canvas());
  if ($.gl == null)
    return;
  $.lesson = M.Lesson1$();
  t1 = document.querySelector("#update");
  t1.toString;
...
tick: function(time) {
  ...
  t1 = $.lesson;
  t1.toString;
  t2 = J.get$width$x($.get$canvas());
  t3 = J.get$height$x($.get$canvas());
  t4 = J.get$width$x($.get$canvas());
  t5 = J.get$height$x($.get$canvas());
  if (typeof t4 !== "number")
    throw t4.$div();
  if (typeof t5 !== "number")
    throw H.iae(t5);
  t1.drawScene$3(t2, t3, t4 / t5);
},
...
w0: function(x, err) {
  ...
  for (w = x; true; w = wNew) {
    if (typeof w !== "number")
      H.throwExpression(new P.ArgumentError(w));
    ew = Math.exp(w);
    t1 = J.getInterceptor$n(w);
    wNew = t1.$sub(w, J.$div$n(J.$sub$n(t1.$mul(w, ew), x), J.$add$ns(t1.$mul(w, ew), ew)));
    if (J.abs$0$n(t1.$sub(w, wNew)) <= err)
      break;
  }
  return w;
},

makeLogarithmicSpiral: function(theta, resolution, a, b) {
  var points, t1, t2, cur_theta, prevx, prevy, d_theta, this_theta, t3, r, t4, p, d, line;
  points = P.List_List(null, null);
  if (typeof theta !== "number")
    throw H.iae(theta);
  t1 = J.getInterceptor$n(a);
  ...
  d_theta = 1;
  for (; cur_theta < theta;) {
    this_theta = 3.141592653589793 * cur_theta / 180;
    t3 = t2.$mul(b, this_theta);
    if (typeof t3 !== "number")
      H.throwExpression(new P.ArgumentError(t3));
    r = t1.$mul(a, Math.pow(2.718281828459045, t3));
    t3 = J.getInterceptor$n(r);
    t4 = t3.$mul(r, Math.cos(this_theta));
    p = [t4, t3.$mul(r, Math.sin(this_theta)), 0];
    C.JSArray_methods.addAll$1(points, p);
    t3 = p[0];
    t4 = p[1];
    t3 = J.$sub$n(prevx, t3);
    if (typeof t3 !== "number")
    ...
    if (typeof resolution !== "number")
      throw H.iae(resolution);
    if (d >= resolution) {
      t3 = J.$div$n(M.w0(J.$div$n(t2.$mul(b, resolution), a), 0.000001), b);
      if (typeof t3 !== "number")
        throw H.iae(t3);
      d_theta *= t3;
      line = C.JSNumber_methods.toString$0(d_theta);
      H.printToConsole(line);
    }
    cur_theta += d_theta;
    prevx = p[0];
...
GlProgram$: function(fragSrc, vertSrc, attributeNames, uniformNames) {
  var t1 = new M.GlProgram(P.LinkedHashMap_LinkedHashMap(null, null, null, J.JSString, J.JSInt), P.LinkedHashMap_LinkedHashMap(null, null, null, J.JSString, P.UniformLocation), null, null, null);
...
main_closure: {"": "Closure;",
  call$1: function($event) {
    return $.lesson.update$0();
  },
  $is_args1: true
},

Lesson1: {"": "Object;program,points,triangleVertexPositionBuffer,squareVertexPositionBuffer,lastTime",
  drawScene$3: function(viewWidth, viewHeight, aspect) {
    var t1, t2, t3;
    J.viewport$4$x($.gl, 0, 0, viewWidth, viewHeight);
    J.clear$1$ax($.gl, 16640);
    J.enable$1$x($.gl, 2929);
    J.disable$1$x($.gl, 3042);
    $.pMatrix = T.makePerspectiveMatrix(45, aspect, 0.1, 100);
    t1 = $.get$mvStack();
    t2 = $.mvMatrix;
    ...
    t1.push(t3);
    t3 = $.mvMatrix;
    t3.translate$3(t3, 0, 0, -7);
    J.bindBuffer$2$x($.gl, 34962, this.triangleVertexPositionBuffer);
    t3 = this.program.attributes;
    J.vertexAttribPointer$6$x($.gl, t3.$index(t3, "aVertexPosition"), 3, 5126, false, 0, 0);
    this.setMatrixUniforms$0();
    t3 = $.gl;
    t1 = this.points.length;
    if (typeof 3 !== "number")
      throw H.iae(3);
    J.drawArrays$3$x(t3, 3, 0, C.JSInt_methods.$tdiv(t1, 3) - 1);
    t1 = $.get$mvStack();
    if (0 >= t1.length)
      throw H.ioore(t1, 0);
    $.mvMatrix = t1.pop();
  ...
  setMatrixUniforms$0: function() {
    var t1 = this.program.uniforms;
    J.uniformMatrix4fv$3$x($.gl, t1.$index(t1, "uPMatrix"), false, $.pMatrix.storage);
    t1 = this.program.uniforms;
    J.uniformMatrix4fv$3$x($.gl, t1.$index(t1, "uMVMatrix"), false, $.mvMatrix.storage);
  },
  update$0: function() {
    var theta, p, t1, t2;
    theta = H.Primitives_parseDouble(H.interceptedTypeCast(document.querySelector("#degrees"), "$isInputElement").value, null);
    p = M.makeLogarithmicSpiral(theta, H.Primitives_parseDouble(H.interceptedTypeCast(document.querySelector("#resolution"), "$isInputElement").value, null), H.Primitives_parseDouble(H.interceptedTypeCast(document.querySelector("#a"), "$isInputElement").value, null), H.Primitives_parseDouble(H.interceptedTypeCast(document.querySelector("#b"), "$isInputElement").value, null));
    P.print(theta);
    this.points = p;
    J.bindBuffer$2$x($.gl, 34962, this.triangleVertexPositionBuffer);
    t1 = $.gl;
    t2 = new Float32Array(this.points);
    t2.$dartCachedLength = t2.length;
    J.bufferDataTyped$3$x(t1, 34962, t2, 35044);
    M.tick(this.lastTime + 1);
  },
  Lesson1$0: function() {
    var t1, t2;
    this.program = M.GlProgram$("          precision mediump float;\r\n\r\n          void main(void) {\r\n              gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0);\r\n          }\r\n        ", "          attribute vec3 aVertexPosition;\r\n\r\n          uniform mat4 uMVMatrix;\r\n          uniform mat4 uPMatrix;\r\n\r\n          void main(void) {\r\n              gl_Position = uPMatrix * uMVMatrix * vec4(aVertexPosition, 1.0);\r\n          }\r\n        ", ["aVertexPosition"], ["uMVMatrix", "uPMatrix"]);
    J.useProgram$1$x($.gl, this.program.program);
    this.triangleVertexPositionBuffer = J.createBuffer$0$x($.gl);
    this.points = M.makeLogarithmicSpiral(2000, 0.05, 0.5, 0.05);
    J.bindBuffer$2$x($.gl, 34962, this.triangleVertexPositionBuffer);
    t1 = $.gl;
    t2 = new Float32Array(this.points);
    t2.$dartCachedLength = t2.length;
  ...
  static: {
Lesson1$: function() {
  var t1 = new M.Lesson1(null, null, null, null, 0);
  t1.Lesson1$0();
  return t1;
}}
...
Matrix4: {"": "Object;storage<",
  ...
  translate$3: function(_, x, y, z) {
    ...
    t16 = t1[11];
    t17 = t1[15];
    t1[12] = t2 * x + t3 * y + t4 * z + t5 * 1;
    t1[13] = t6 * x + t7 * y + t8 * z + t9 * 1;
    t1[14] = t10 * x + t11 * y + t12 * z + t13 * 1;
    t1[15] = t14 * x + t15 * y + t16 * z + t17 * 1;
    return this;
  },
  setZero$0: function() {
    var t1 = this.storage;
...
Vector3: {"": "Object;storage<",
  ...
  $add: function(_, other) {
    ...
    t2 = t1[0];
    t3 = other.get$storage();
    t3 = C.JSNumber_methods.$add(t2, t3.$index(t3, 0));
    t2 = t1[1];
    t4 = other.get$storage();
    t4 = C.JSNumber_methods.$add(t2, t4.$index(t4, 1));
    t1 = t1[2];
    t2 = other.get$storage();
    t2 = C.JSNumber_methods.$add(t1, t2.$index(t2, 2));
    t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
    t1 = new T.Vector3(t1);
...
Vector4: {"": "Object;storage<",
  ...
  $add: function(_, other) {
    ...
    t5 = C.JSNumber_methods.$add(t2, t5.$index(t5, 2));
    t1 = t1[3];
    t2 = other.get$storage();
    t2 = C.JSNumber_methods.$add(t1, t2.$index(t2, 3));
    t1 = new Float32Array(4);
    t1.$dartCachedLength = t1.length;
    t1 = new T.Vector4(t1);
...
Isolate.$lazy($, "canvas", "canvas", "get$canvas", function() {
  return document.querySelector("#lesson01-canvas");
});
Isolate.$lazy($, "mvStack", "mvStack", "get$mvStack", function() {
  var t1 = P.List_List(null, T.Matrix4);
  H.setRuntimeTypeInfo(t1, [T.Matrix4]);
  return t1;
});
// Native classes
...
function dart_precompiled($collectedClasses) {
  ...
  function GlProgram(attributes, uniforms, program, fragShader, vertShader) {
  ...
  function Lesson1(program, points, triangleVertexPositionBuffer, squareVertexPositionBuffer, lastTime) {
    this.program = program;
    this.points = points;
  ...
  Lesson1.builtin$cls = "Lesson1";
  if (!"name" in Lesson1)
    Lesson1.name = "Lesson1";
  $desc = $collectedClasses.Lesson1;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Lesson1.prototype = $desc;
